    ...
    if len(fined_cache)==1:
        ...
        if target==pred:  # If the strings are exactly equal
            ...
            try:
                ...
                judge_result=abs(float(target) - float(pred)) < 1e-3
            except:
                # LLM judging
...
class Evaluator:
    def __init__(self,infered_data_path):
        ...
        self.init_data=read_file("MATH/data/all_random_500.jsonl")
    ...
    def eval(self):
        ...
        for dp_id,dp in enumerate(tqdm(self.infered_data["logs"])):
            target=[init_dp["target"] for init_dp in self.init_data if init_dp["id"]==dp["id"]][0]
            # For each turn
    ...
    def report(self):
        ...
        # Save
        print("save to: ",self.evaled_data_path)
